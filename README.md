[![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/manlaya/init-python-amorse-binder/main)

# Python initiation for AMORSE project

This repo contains few notebooks for a quick python initiation for
hydrology within the AMORSE project (BRGM, French Geological Survey;
AEAP, Water Agency of Artois-Picardie).


## Try it in your browser

➡️ **https://mybinder.org/v2/gh/manlaya/init-python-amorse-binder/main**

Non interactive version: https://nbviewer.org/github/manlaya/init-python-amorse-binder/tree/main/notebooks/

## Disclaimer

The python content of this repo is adapted from Python trainings from French Geological Survey
(BRGM), see https://gitlab.com/fsmai/formation-python.

General python part is adapted from Farid Smai & Théophile Guillon (BRGM) works.
Marthe-python part is adapted from Jean-Pierre Vergnes, Ryma Aissat (BRGM) works, and `gridmarthe`
documentation (A. Manlay, BRGM).

## Licence

This work is licensed under the terms of the BSD 3-Clause license. See [LICENSE](LICENSE) for more details.
